...
<?php 
  ...
  <div class="container">
    <center>
        <h3 id="headd"> <label style="color:red;">หมายเหตุ :</label> <label style="color:#1E8449"> สีเขียว = สามารถจองได้</label>
        <label style="color:red">| สีเเดง = ไม่สามารถจองได้ </label>  <label style="color:#8E44AD">| สีม่วง = รอการยืนยันการจองห้อง </label> <label style="color:#6699FF">| สีฟ้า= ยืนยันการจองห้อง </label><!--<label style="color:orange"> | Waiting Confirm = รอเวลา Confirm </label> --></h3>
        <a href="tomorrow.php" id="headd" class="btn btn-primary">วันนี้ : <?php echo $time_nows; ?> จองวันพรุ่งนี้ คลิก >></a><br><br>
        <center><br>
        ...
        <input type="hidden" value="3" id="dd">
              ...
              <tbody>
                <?php 
                    ...
                    echo '<th id="thh">'.$room_name.'</th>';          
                        $sql1 ="SELECT  booking.booking_date, booking.booking_id, booking.user_name, booking.booking_in, booking.booking_out, time.time_in, time.time_out,booking.booking_status FROM time LEFT join booking ON  
                         time.time_in = booking.booking_in and booking.room_id = '$room_id' and booking.booking_day LIKE '%".$date_n->format('Y-m-d')."%' and booking.booking_status='1' 
                        or time.time_in = booking.booking_in and booking.room_id = '$room_id' and booking.booking_day LIKE '%".$date_n->format('Y-m-d')."%' and booking.booking_status='2' 
                        where time.time_status='0' order by time.time_in asc
                         ...
                         while($row=mysqli_fetch_array($result,MYSQLI_ASSOC))
                         {
                            $time_now=date("H:i:s");
                             ...
                             $book_user = $row['user_name'];
                              $time_1=date("H:i:s",strtotime($time_in."  +30 minute"));
                             if($book_user=='adminadmin'){
                                ...
                                if($booking_date==null){
                                    ...
                                    if($time_now>$time_1){
                                        echo '<td id="thh" style="background-color:red;cursor:pointer;"> หมดเวลาจอง</td>';
                                    }else{
                                       /* echo ' <td>ว่าง  <input onclick="deleteData('."$room_id".', '.$time_in.')" id="ima" type="image" alt="Submit" src="images/available.png" name="room_id1" >  </td>';*/
                                        echo "<td id='thh'   style='background-color: #1E8449;cursor:pointer;' onclick=".'"'."deleteData('".$room_id."' ,'".$room_name."', '".$time_in."', '".$time_out."')".'"'." >ว่าง <br></td>";
                                    }
                                    ...
                                    if($booking_status=='2')
                                    {
                                        echo "<td id='thh' style='background-color: #6699FF ;cursor:pointer;'  >จองเเล้ว <br></td>";
                                    }else
                                    {
  ...
  <?php }else{
    ...
    $query_count_hr = "SELECT * FROM `booking` where user_name = '$user_name' and booking_day='$today' and booking_status = '1' or user_name = '$user_name' and booking_day='$today' and booking_status = '2'";
    $result_count_hr=mysqli_query($con,$query_count_hr);
    $count_hr=mysqli_num_rows($result_count_hr);
      

    $query_booking_day = "SELECT * FROM `booking` where user_name = '$user_name' and booking_day='$today' and booking_status = '1' or user_name = '$user_name' and booking_day='$today' and booking_status = '2' order by booking_id asc";
    ...
    $query_booking_day2 = "SELECT * FROM `booking` where user_name = '$user_name' and booking_day='$today' and booking_status = '1' or user_name = '$user_name' and booking_day='$today' and booking_status = '2' order by booking_id desc";
...
<div class="container">
    <center>
        ...
        <a href="tomorrow.php" id="headd" class="btn btn-primary">วันนี้ : <?php echo $time_nows; ?> จองวันพรุ่งนี้ คลิก >> </a>
        <center><br>
        <select onchange="show_res_order(this.value)">
            <option value="">เลือกชั้น</option>
            <option value="1"> ชั้น 1 อาคาร 1</option>
                ...
                <?php 
                   $sql2 = "SELECT * FROM `room` where room_status= '0' $po";
                    ...
                    echo '<th id="thh">'.$room_name.'</th>';          
                        $sql1 ="SELECT  booking.booking_date, booking.booking_id, booking.user_name, booking.booking_in, booking.booking_out, time.time_in, time.time_out,booking.booking_status FROM time LEFT join booking ON    
                        time.time_in = booking.booking_in and booking.room_id = '$room_id' and booking.booking_day LIKE '%".$date_n->format('Y-m-d')."%' and booking.booking_status='1' 
                        or time.time_in = booking.booking_in and booking.room_id = '$room_id' and booking.booking_day LIKE '%".$date_n->format('Y-m-d')."%' and booking.booking_status='2'  where time.time_status='0'order by time.time_in asc
                            ...
                            $time_now=date("H:i:s");
                             ...
                             if($book_user=='adminadmin'){
                                ...
                                if($booking_date==null){
                                    ...
                                    if($time_now>$time_1){
                                        echo '<td id="thh" style="background-color: red;cursor:pointer;"> หมดเวลาจอง <br></td>';
                                    }else{
                                       /* echo ' <td>ว่าง  <input onclick="deleteData('."$room_id".', '.$time_in.')" id="ima" type="image" alt="Submit" src="images/available.png" name="room_id1" >  </td>';*/
                                        echo "<td id='thh' style='background-color: #1E8449;cursor:pointer;' onclick=".'"'."deleteData('".$room_id."' ,'".$room_name."', '".$time_in."', '".$time_out."')".'"'." >ว่าง <br></td>";
                                    }
                                    ...
                                    if($user_name==$book_user){
                                        if($booking_status=='2'){
                                            //รับกุญเเจเเล้ว
                                            echo "<td id='thh' style='background-color:#6699FF;cursor:pointer;' >".$book_user." <br></td>"; 
                                        }else{
                                        ...
                                        if($booking_status=='2')
                                        {
                                            echo "<td id='thh' style='background-color:#6699FF ;cursor:pointer;'  > จองแล้ว<br></td>";
                                        }else
                                        {
                                            echo "<td id='thh' style='background-color:#8E44AD ;cursor:pointer;'  >จองแล้ว <br></td>";
              ...
              <tbody>
                <?php 
                   $sql2 = "SELECT * FROM `room` where room_status= '0'$po";
                    ...
                    echo ' <tr>';
                    echo '<th id="thh">'.$room_name.'</th>';          
                        $sql1 ="SELECT  booking.booking_date, booking.booking_id, booking.booking_status, booking.user_name, booking.booking_in, booking.booking_out, time.time_in, time.time_out,booking.booking_status FROM time LEFT join booking ON   
                        time.time_in = booking.booking_in and booking.room_id = '$room_id' and booking.booking_day LIKE '%".$date_n->format('Y-m-d')."%' and booking.booking_status='1' 
                        or time.time_in = booking.booking_in and booking.room_id = '$room_id' and booking.booking_day LIKE '%".$date_n->format('Y-m-d')."%' and booking.booking_status='2'  where time.time_status='0'order by time.time_in asc
                             ...
                             if($book_user=='adminadmin'){
                                ...
                                if($booking_date==null){
                                    ...
                                    if($time_now>$time_1){
                                       ...
                                       /* echo ' <td>ว่าง  <input onclick="deleteData('."$room_id".', '.$time_in.')" id="ima" type="image" alt="Submit" src="images/available.png" name="room_id1" >  </td>';*/
                                /*        echo "<td onclick=".'"'."deleteData1('".$room_id."', '".$time_in."', '".$time_out."')".'"'."></td>";*/
                                        
                                        echo "<td id='thh'  onclick=".'"'."deleteData1('".$room_id."' , '".$time_in."', '".$time_out."')".'"'." style='background-color:#1E8449;cursor:pointer;'  >ว่าง <br></td>";
                                    }

                                }else{
                                   if($booking_status=='2')
                                   {
                                       echo "<td id='thh'  style='background-color: #6699FF ;cursor:pointer;'  onclick=".'"'."goto('".$book_user."')".'"'.">".$book_user." <br></td>";
                                   }else
                                   {                                      
                                        echo "<td id='thh' style='background-color: #8E44AD ;cursor:pointer;'  onclick=".'"'."goto('".$book_user."')".'"'.">".$book_user." <br></td>";                                        
                                   }
...
<?php } }?>
          ...
          if(b_in==t){
            ...
            if(xx<=0){
                ...
                return false;       
                     ...
                     if (confirm("ยืนยันที่จะจองห้องที่ " +name+ " เวลา " +t+ " หรือไม่")) {
                        if (xmlHttp != null) {
                            xmlHttp.open("GET", "insert_book.php?id="+id+"&ro="+t+"&o="+o , true);
                            xmlHttp.send();
	...
	<script>
            ...
            function deleteData1(id,t,o) {		
                window.location = "input_name.php?id="+id+"&ro="+t+"&o="+o;
            }
	          
        function goto(id) {		
                window.location = "history_booking_user.php?id="+id;
            }
