	...
	$user_name=$_SESSION['login'];
            ...
            if($_SESSION['status']=='2'){
                ...
                $time_now=date("Y-m-d");
					...
					$date_n = new DateTime($time_now);
					$start = new DateTimeImmutable($date_n->format('Y/m/d'));
					$datetime = $start->modify('+1 day');
                
              
                $query_booking = "SELECT * FROM `booking` where room_id = '$room_id' and booking_in = '$t' and booking_day LIKE '%".$time_now."%' and booking_status='1' or room_id = '$room_id' and booking_in = '$t' and booking_day LIKE '%".$time_now."%' and booking_status='2'";
                $result2=mysqli_query($con,$query_booking);
                $count_hr_admin=mysqli_num_rows($result2);
        
                            $query_room = "SELECT  admin_booking.stu_id FROM admin_booking inner join booking ON  booking.booking_id = admin_booking.booking_id AND booking.booking_day LIKE '%".$time_now."%' where booking.user_name = '$user_name' or admin_booking.stu_id = '$user_name' order by booking.booking_id";
            ...
            /*                            $query_room1 = "SELECT * FROM `booking` where  booking_day LIKE '%".$time_now."%' and user_name = '$user_name' and booking_status='1' or booking_day LIKE '%".$time_now."%' and user_name = '$user_name' and booking_status='2'";
                            ...
                            echo 'วันนี้คุณได้ทำการจองห้องครบ 3 ชั่วโมงเเล้ว';
                                        ...
                                        $sql_booking = "INSERT INTO `booking` (`booking_id`, `room_id`, `booking_day`, `booking_in`, `booking_out`, `booking_date`, `booking_status`, `user_name`) VALUES (NULL, '$room_id', '".$time_now."',  '$t', '$o', Now(), '1', '$user_name');";
...
/*                    }else{
                          echo 'วันนี้คุณได้ทำการจองห้องเเล้ว';
                    }*/
